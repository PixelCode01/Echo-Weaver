...
class WaveManager:
    def __init__(self):
        ...
        self.enemy_speed = ENEMY_SPEED # Initialize enemy speed
    ...
    def start_next_wave(self):
        ...
        self.enemy_speed = ENEMY_SPEED + (self.current_wave - 1) * 0.2 # Update speed for the new wave
    ...
    def update(self, enemies_group, core):
        ...
        if spawn_rate < 10: # Minimum spawn rate
            spawn_rate = 10
    ...
    def _spawn_enemy(self, enemies_group):
        ...
        if self.current_wave >= 10 and enemy_type_roll < 0.05: # Disruptor enemies from wave 10
            enemies_group.add(DisruptorEnemy())
        elif self.current_wave >= 9 and enemy_type_roll < 0.05: # Spawner enemies from wave 9
            enemies_group.add(SpawnerEnemy())
        elif self.current_wave >= 8 and enemy_type_roll < 0.08: # Healer enemies from wave 8
            enemies_group.add(HealerEnemy())
        elif self.current_wave >= 7 and enemy_type_roll < 0.1: # Shielded enemies from wave 7
            enemies_group.add(ShieldedEnemy())
        elif self.current_wave >= 5 and enemy_type_roll < 0.15: # Splitter enemies from wave 5
            enemies_group.add(SplitterEnemy())
        elif self.current_wave >= 3 and enemy_type_roll < 0.15: # Ghost enemies from wave 3
            enemies_group.add(GhostEnemy())
        elif self.current_wave >= 5 and enemy_type_roll < 0.1: # Charger enemies from wave 5
            enemies_group.add(ChargerEnemy())
        elif enemy_type_roll < 0.3: # Zigzag enemies always have a chance
